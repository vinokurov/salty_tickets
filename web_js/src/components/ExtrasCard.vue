<template>
    <b-card no-body :bg-variant="cardStyle.bg" :text-variant="cardStyle.text"
        ...
        <div class="card-header text-light" :style="'background-color: '+headerColor">
          ...
          <h5 class="card-title">
              <font-awesome icon="lock" v-if="!product.editable"/> 
              <font-awesome :icon="product_icon" v-else/> 
              {{title}}
              </h5>
        </div>
        <div class="card-body">
...
export default {
  ...
  computed: {
    ...
    product_icon: function() {
        if (this.product.key == 'sunday_roast') {
            return 'utensils'
        } else {
            return 'ship'
        }
